...
var View = function() {
	...
	this.printStatus = function(obj, timestamp) {
		...
		console.log(t + self.getStatusLabel(obj.status, '\u25CF ') + obj.displayName() + ' (' + obj.jid +  ') is ' + obj.status + show);
	...
	this.printMessage = function(jid, msg) {
		...
		if (msg.action==='from') {
			console.log(wrap(t + contact.displayName() + ' >>> ' + this.formatSmileys(msg.text, c)));
